...
use super::lex::{scan, SrcPos, SrcSpan, Token, TokenType};
...
pub fn from_tokens(toks: &[Token]) -> Result<Vec<Sexpr>, Error> {
    let mut seq = Vec::new();
...
fn test_from_tokens() {
    // leaf
    assert_eq!(
        from_tokens(&scan("a").unwrap()).unwrap(),
    ...
    assert_eq!(
        from_tokens(&scan("(a (b))").unwrap()).unwrap(),
        vec![Sexpr::Compound(
            vec![
                Sexpr::Atom(Token {
                    t: TokenType::Identifier,
    ...
    // sequence
    assert_eq!(
        from_tokens(&scan("a (b) c").unwrap()).unwrap(),
        vec![
            Sexpr::Atom(Token {
                t: TokenType::Identifier,
                span: ((0, 0), (0, 0)),
