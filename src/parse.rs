...
use super::lex::{SrcSpan, Token, TokenType};
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::SexprError(err) => write!(f, "sexpr parse error: {:?}", err),
...
fn parse_sexpr_seq(sexpr_seq: &[Sexpr]) -> Result<Vec<Rc<RefCell<Expr>>>, Error> {
...
fn parse_compound(children: &Vec<Sexpr>, span: SrcSpan) -> Result<Expr, Error> {
...
pub fn parse(toks: &[Token]) -> Result<Vec<Rc<RefCell<Expr>>>, Error> {
    let seq = match sexpr::from_tokens(toks) {
        Ok(seq) => seq,
        Err(err) => return Err(Error::SexprError(err)),
    };
    parse_sexpr_seq(&seq)
}
